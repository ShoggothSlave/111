...
func _on_area_banque_50_piasse_body_entered(_body: Node2D) -> void:
	if vide == false:
		if rest <= 0:
		...
		if rest > 0:
			if _body is player:
				rest = rest - 1
				ManagerPlayer.money = ManagerPlayer.money + 50
...
func _on_area_banque_frappe_50_piasse_left_body_entered(_body: Node2D) -> void:
	if vide == false:
		if _body is player:
			kickable_left = true

func _on_area_banque_frappe_50_piasse_left_body_exited(_body: Node2D) -> void:
	kickable_left = false

func _on_area_banque_frappe_50_piasse_right_body_entered(_body: Node2D) -> void:
	if vide == false:
		if _body is player:
			kickable_right = true

func _on_area_banque_frappe_50_piasse_right_body_exited(_body: Node2D) -> void:
